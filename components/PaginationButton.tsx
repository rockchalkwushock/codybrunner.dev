...
export const PaginationButton: React.FC<Props> = ({ direction, slug }) => {
  return (
    ...
    >
      {direction === 'prev' && (
        <Rewind className="h-8 group-hover:text-fuchsia-400 dark:group-hover:text-fuchsia-600 w-8" />
      )}
      {direction === 'next' && (
        <FastForward className="h-8 group-hover:text-fuchsia-400 dark:group-hover:text-fuchsia-600 w-8" />
      )}
    </PostLink>
  )
}
