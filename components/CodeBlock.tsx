      ...
      {({ className, style, tokens, getLineProps, getTokenProps }) => {
        return (
          <pre className={`${className} group relative`} style={style}>
            {status === 'inactive' ? (
              <Copy
                className="absolute cursor-pointer opacity-0 right-4 top-4 group-hover:opacity-100"
                onClick={copy}
              />
            ) : status === 'copied' ? (
              <CheckCircle className="absolute right-4 top-4" />
            ) : (
              <XCircle className="absolute right-4 top-4" />
            )}
            {tokens.map((line, i) => (
