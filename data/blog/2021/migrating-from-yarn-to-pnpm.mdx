...
However this was not the case for this website. I continuously received and error message about `esbuild` could not be found or that the versions were in conflict. I finally just branched off of staging and removed the majority of the source code, `node_modules`, and emptied the `package.json`. Turns out that `esbuild` was not being installed at the top level anymore as `mdx-bundler` was expecting and thus the build would fail. There went 3 hours of my day 🤦🏻‍♂️.
...
As stated at the beginning of this post I avoided moving to XXX because of the cognitive load, pnpm is nearly a one-for-one with classic Yarn but I really really did not want to have to remember that I no longer have Yarn on my machine so I updated the aliases in my `.zshrc` to use `pnpm` instead
