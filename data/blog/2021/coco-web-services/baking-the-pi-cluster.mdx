...
If you don't already have the [Raspberry Pi Imager](https://www.raspberrypi.com/software/) you can find the download for it at the previous link or you can install it via [Homebrew](https://brew.sh):

```bash
brew install raspberry-pi-imager
```

![Miscellaneous Utility Images](/images/2021/coco-web-services/baking-the-pi-cluster/misc-utility-images.png)

![Bootloader](/images/2021/coco-web-services/baking-the-pi-cluster/bootloader.png)

![USB Boot](/images/2021/coco-web-services/baking-the-pi-cluster/usb-boot.png)
...
![RaspberryPi OS Other](/images/2021/coco-web-services/baking-the-pi-cluster/rpios-other.png)

![RaspberryPi OS Lite 32 Bit](/images/2021/coco-web-services/baking-the-pi-cluster/rpios-32-bit.png)

![Customize Install](/images/2021/coco-web-services/baking-the-pi-cluster/custom-options.png)

![Customize Install 2](/images/2021/coco-web-services/baking-the-pi-cluster/custom-options-2.png)
...
Even though I was able to bootstrap somethings with the OS via the options menu in the Raspberry Pi Imager there were still some things to be done in the configuration:
...
1. Updated the Locales
2. Updated the Country for WLAN
3. Added USB Boot as the leading boot order

![Localization Menu](/images/2021/coco-web-services/baking-the-pi-cluster/localization.png)

![Locales Menu](/images/2021/coco-web-services/baking-the-pi-cluster/locale.png)

![WLAN Country Menu](/images/2021/coco-web-services/baking-the-pi-cluster/wlan-country.png)

![Advanced Options Menu](/images/2021/coco-web-services/baking-the-pi-cluster/advanced-options.png)

![Boot Order Menu](/images/2021/coco-web-services/baking-the-pi-cluster/boot-order.png)

![USB Boot Config](/images/2021/coco-web-services/baking-the-pi-cluster/usb-boot-config.png)
...
Some Googling on how to do backups for images led me to finding the wonderful software called [ApplePi-Baker](https://www.tweaking4all.com/hardware/raspberry-pi/applepi-baker-v2/) that works like a charm for this. I installed the software using [Homebrew](https://brew.sh):

```bash
brew install applepi-baker
```

![Select Drive](/images/2021/coco-web-services/baking-the-pi-cluster/applepi-baker-initial.png)

![Backing Up](/images/2021/coco-web-services/baking-the-pi-cluster/applepi-baker-2.png)
...
```bash:macOS
ssh pi@rpi_one.local
# ssh: connect to host rpi_one.local port 22: Connection refused
...
If you want to read up more on SSH and learn more I highly suggest the following [website](https://www.ssh.com).
...
> One thing I did realize is my `.bash_aliases` are living on the `pi` account so I needed to copy those over to my user `cody` or just recreate them under that account.
...
```bash:RPiOS
sudo usermod pi --expiredate 1
```

This truly disables the account by telling the system the user's account expired on Jan 2, 1970. I was able to verify the change by trying to execute a command as `sudo`:
...
ssh root@rpi_one.local -p 122 # new password
# all should fail
```
...
Main points to take away from the below configuration:

- I am only using IPv4 connections so that's why you see `inet`.
- I am not allowing any port forwarding or tunneling.
- I explicitly say where to find the `authorized_keys`.
- As added security I deny `root` and `pi` from being allowd to login.
- I limit authentication tries to 3 times.
- No password based authentication (ssh-keys only).
- No empty passwords.
- No `root` login.
- A custom SSH port.
...
I updated my `.bash_aliases` with the following:
...
alias nodes="sudo docker node ls";
```
...
```bash:RPiOS
d version # docker version
dr hello-world # docker run hello-world
d system prune # docker system prune
dil # docker image ls
# copy image id
dirm <id> # docker image rm <id>
```
...
I only have 2 Pi's at the moment and my cluster case supports 4. I don't think I will expand past 2 Pi's anytime soon; however all the steps in this post will need to be repeated for each Pi. The only real change needed will be to change the hostname of each Pi for clustering and then run the above `docker swarm join` command with the generated token and the IP address of the manager node with the `2337` tacked on at the end. I had made that initial backup of the first Pi from ApplePi Baker and was able to start from that point thanks to the feature of creating from a custom image:

![Use Custom Image](/images/2021/coco-web-services/baking-the-pi-cluster/use-custom-image.png)

![From Backup Image](/images/2021/coco-web-services/baking-the-pi-cluster/from-backup.png)
